...
- `pkg/version` — the build identity every binary reports; one git tag stamped into all targets by `scripts/build.sh` (the submodules under client/ import it through `replace ../..`, so they cannot drift into a second scheme). [Version, String]
- `pkg/zapret` — model nfqws instances as the managed unit (one process / NFQUEUE / port set / switchable strategy). Also the tuner's SANDBOX: `Sandbox` owns a table that queues only probe-marked egress to its own NFQUEUE plus a second engine bound to it, so a candidate strategy is measured against live DPI without anyone else's traffic meeting it; `GenerateNft` emits the matching skip rule in the production table. `Apply` has the signature `desynctune.TuneService` takes. `capture.go` DERIVES the queue's port set from a composed argv (`CaptureFromArgs`) instead of holding a second literal — two independent statements about the same traffic is how Discord voice filtered ports the queue never carried. [Instance, Capture, Sandbox, GenerateNft, CaptureFromArgs, GenerateIsolateNft, ExecLauncher, TuneMark]
- `pkg/zaptune` — per-RULE recipe tuning glue; `ComposePinned` honours a chain step's `strategy_id` as a real pin and REPORTS one it cannot honour (a silent pin turns an experiment into a measurement of something else). `recipeRenderable` accepts a bounded port range plus `--filter-l7` as a scope, because raw UDP has no SNI to scope by: "service has blocked domains" → "its zapret rung uses recipe X"; keeps service as the unit (one recipe, one --new block, all its domains).
...
- `cmd/presetgen` — convert an unpacked upstream release into `docs/presets/*.args` via `strategyimport.AsPreset`, so re-reading a new bundle is a diff rather than an act of memory. Names a launcher it cannot render instead of skipping it silently.
...
- `pkg/prospect` — strategies THIS BOX proved for itself, persisted. A tuner win is a lead, not a recipe: only a second win in a separate pass confirms it into the composer's pool, and repeated losses demote it back out. The hedge against every recipe coming from a bundle somebody else maintains. [Store, Open, Won, Lost, Recipes]
- `pkg/desynctune` — v7 tuner: turn generated strategies into an A/B, ask tester which beats the no-desync baseline. `TwoPhase` runs a cheap loss-only pass over EVERY candidate then a throughput pass over the few survivors (one pass cannot do both: loss alone crowns a recipe that connects and crawls, throughput on all of them spends tens of minutes of uplink). `Gate` admits one pass at a time — the sandbox is a singleton — with a per-service cooldown and global spacing, stamped on release regardless of outcome. Apply comes from `zapret.Sandbox`. **Still unwired: nothing in the daemon calls it yet.** [TwoPhase, Gate, Candidates, StrategyID]
...
- `pkg/strategyimport` — read third-party strategy bundles (Flowseal `.bat`, shell, fenced markdown) ONCE and render them two ways. **Recipes** (`Import`): one profile per rule, deployment detail dropped, payload paths to basenames, host selectors to `{{DOMAINS}}`, `--ipset` refused whole; IDs derive from the normalized args so the KB keeps its learning across bundle updates. **Presets** (`AsPreset`, `preset.go`): the bundle kept WHOLE — the author's profiles, order and per-profile hostlists — because a nine-profile bundle rendered one profile per rule is a different strategy under the same name, which is precisely why ALT12 is refused as self-shadowing. Both renderings share the reader; do not write a third. **Not yet wired to `pkg/flowseal`** — an update still does not re-read what it brought. [Import, ImportDir, Normalize, Techniques, AsPreset, RenderPreset, PresetName]
