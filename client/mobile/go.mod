// The Android app is a SEPARATE module so sing-box's very large dependency graph
// (libbox pulls in the whole box: every protocol, quic, wireguard, gVisor…) never
// touches the root module, which stays at its current two direct dependencies.
// Built with SagerNet's gomobile FORK: ONE `gomobile bind` binds BOTH
// github.com/sagernet/sing-box/experimental/libbox and this package.
//
// BUILD TAGS ARE MANDATORY. sing-box gates whole subsystems behind them, and the
// default (untagged) build compiles cleanly into an app that cannot work:
//
//	gomobile bind -tags with_clash_api,with_quic,with_utls,with_gvisor ...
//
//	with_clash_api  the ENTIRE Lotsman control plane steers sing-box over the
//	                loopback Clash API. Without this tag sing-box links a stub
//	                whose clash-server constructor returns "clash api is not
//	                included in this build", so every start fails. Measured, not
//	                assumed — see TestGeneratedAndroidConfigLoadsInLibbox, which
//	                fails with exactly that message when the tag is absent.
//	with_quic       hysteria2 / tuic / QUIC transports.
//	with_utls       the tls.utls fingerprint the config's uTLSFingerprint sets.
//	with_gvisor     the gvisor/mixed tun stacks (we default to "system", but a
//	                config may ask for the others).
//
// with_ech must NOT be passed: it was folded into stdlib and sing-box 1.13.14
// deliberately fails to compile when it is set.
module github.com/strace-me/lotsman/client/mobile
