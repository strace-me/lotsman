#!/bin/sh
# Flowseal ALT12 -> nfqws (Linux). Source: zapret-discord-youtube 1.9.9a.
# winws-only --wf-tcp/--wf-udp dropped (replaced by nft NFQUEUE); 2 game-filter
# profiles dropped (no GameFilter on Linux yet). Usage: alt12.sh [qnum].
QNUM="${1:-200}"
...
--filter-udp=19294-19344,50000-50100 --filter-l7=discord,stun --dpi-desync=fake --dpi-desync-fake-discord="$B/stun.bin" --dpi-desync-fake-discord="$B/quic_initial_dbankcloud_ru.bin" --dpi-desync-fake-stun="$B/quic_initial_dbankcloud_ru.bin" --dpi-desync-repeats=3 --new \
...
--filter-tcp=443 --hostlist="$L/list-google.txt" --ip-id=zero --dpi-desync=hostfakesplit --dpi-desync-fooling=ts --dpi-desync-hostfakesplit-mod=host=www.google.com --new \
