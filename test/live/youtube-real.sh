...
if "${CURL[@]}" --http3 -o /dev/null -w '' https://www.youtube.com/ 2>/dev/null; then
  ...
  row "почему это важно" "у ютуба своя проба ТОЛЬКО по TCP; 'TCP ok, QUIC dead' — записанная у нас ловушка LOT-3"
fi
...
Как читать:
  ...
  D хорошо, а в браузере рвётся             → дело не в пути, а в QUIC (E)
      или в отличии нашего TLS-приветствия от браузерного.
